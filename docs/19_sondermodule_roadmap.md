...
|---|---|---|---|---|
| Kuechenlicht | `net_erl` | Relais-Lichtsteuerung plus Praesenz per bevorzugtem `HLK-LD2410C`, `HLK-LD2410B` nur als Alt-/Testvariante; Feuchte/Temperatur ueber `BME280` oder `SHT41`; Luftqualitaet ueber `ENS160`; Lichtwert ueber `VEML7700`, der Auto-Ein bei hellem Raum unterdruecken darf; 17er NeoPixel-Ring fuer spaetere Luftqualitaets-/Wertanzeige und einfache Animation | kein fertiges Sensor-Set entschieden, kein Radar-/Licht-/AQ-Hardwarebeleg, kein LED-Ring-Beweis | `net_erl`-Basis real belegt, Sondermodul nur architekturvorbereitet |
...
| `net_sen`-Umweltknoten | `net_sen` | breitere Umwelt- und Luftqualitaetssensorik; spaeter optional Display, aber nicht zwingend | Display ist kein Muss; Sensorwahl bleibt diszipliniert statt Alles-auf-ein-Board | `net_sen` mit DHT22 real belegt; `BMP280`/`BME280`/`ENS160`/`PIR` compile/stub vorbereitet; `SHT41`, `VEML7700`, Radar, Display nur architekturvorbereitet |
