...
## Sprintziel
Gemeinsame Doku nach dem Fake-Master-Serverlauf, der getrennten `[SIM]`-Dashboard-Sicht und dem jetzt belegten `bat_sen`-Server-/Langlaufstand repo-weit widerspruchsfrei halten, ohne Restluecken zu verschweigen oder aufzublaehen.
...
- den `bat_sen`-Stand ehrlich halten: lokaler ADC-Pfad, offizieller Serverpfad und fokussierter Langlauf sind belegt; endgueltige Profilwahl, Deep-Sleep und Mehrgeraetebetrieb bleiben offen
...
- `bat_sen_01` ist auf realem Board jetzt lokal bis ADC-/Handshake-Basis, offiziell bis Broker/SQLite/Influx und zusaetzlich in einem fokussierten Langlauf belegt
- offen bleiben vor allem endgueltige `bat_sen`-Profilwahl, Deep-Sleep-/Mehrgeraetebetrieb, weitere offizielle Pfade jenseits der belegten Einzelgeraete und ein vollstaendiger Gesamtprojektnachweis
...
3. Die echten Restpunkte nach dem belegten `bat_sen`-Pfad sind explizit benannt statt weichformuliert.
